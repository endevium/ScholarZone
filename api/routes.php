...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_GET['route'] === 'applicantsupdateProfile') {
    $input = json_decode(file_get_contents("php://input"), true);

    if (!isset($input['token'])) {
        echo json_encode(["status" => 400, "message" => "Token is required."]);
        exit;
    }

    $controller = new ApplicantController($db);
    $response = $controller->updateProfile($input['token'], $input);
    echo json_encode($response);
    exit;
}


if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_GET['route'] === 'reviewersupdateProfile') {
    $input = json_decode(file_get_contents("php://input"), true);
